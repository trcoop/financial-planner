 ...
 *
 * Two traps when checking this table, both hit during the FIN-65 review:
 *
 * 1. Verifying a SINGLE year against published one-decimal index levels does not work: 32.4/31.5
 *    for 1966 carries about +/-0.3pp of rounding error, wider than the disagreements such a check
 *    is trying to resolve. Reconstructing the level and comparing THAT is well conditioned.
 * 2. Compounding all 98 rates and checking the endpoint proves less than it appears. Each rate is
 *    `index[Y]/index[Y-1] - 1`, so the product telescopes exactly to `index[2025]/index[1927]`:
 *    it validates the two endpoints and is blind to every interior value, and offsetting per-year
 *    errors cancel out of it entirely.
 *
 * What the series is actually validated against is the decade-checkpoint reconstruction in
 * `inflationData.test.ts`: the price level rebuilt year by year from a 1927 base of 17.4 lands
 * within 0.03% of the published BLS annual average at 1940, 1950, ... 2020 and 2024. A corrupted
 * year throws off every checkpoint after it.
 *
 * Note ProjectionLab reports 1966 as 3.46% and 1932 as -10.27% because they use
 * December-to-December CPI; this table is annual-average, so per-year differences against them
 * are a convention difference, not an error.
 *
